...
exchange = 'poloniex'
MARKET_URL = "https://poloniex.com/public"
headers = {
...
def get_ticker_merged(symbol, mode):
	try:
		params_ticker = {'command': 'returnTicker'}
		params_book = {'command': 'returnOrderBook', 'currencyPair': symbol, 'depth': 1}
		myticker = eval(http_get_request(MARKET_URL, headers, params_ticker))[symbol]
		orderbook = eval(http_get_request(MARKET_URL, headers, params_book))
		dt, ms = time_transfer(get_time())
		ticker = {'ts': dt, 'nsec': ms, 'symbol': symbol, 'bid': orderbook['bids'][0], 'ask': orderbook['asks'][0], 'last_price': myticker['last'], 'volume': myticker['baseVolume'], 'turnover': 0}
		if mode:
...
#	get_ticker_merged('USDT_BTC', '')
	print_ticker('USDT_BTC')
#	send_client('USDT_BTC', '')
